...
/**
 * Leeloocert module upgrade code.
 *
 ...
 */
function xmldb_leeloocert_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2016120503) {
        ...
        $table = new xmldb_table('leeloocert_templates');
        $field = new xmldb_field('name', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, 'id');
        $dbman->change_field_precision($table, $field);

        // Savepoint reached.
        upgrade_mod_savepoint(true, 2016120503, 'leeloocert');
    }

    if ($oldversion < 2016120505) {
        $table = new xmldb_table('leeloocert');
        ...
        $field = new xmldb_field('certificatevalidthru', XMLDB_TYPE_TEXT, null, null, null, null, null, 'emailothers');
        ...
        $table = new xmldb_table('leeloocert_issues');
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2016120505, 'leeloocert');
    }

    if ($oldversion < 2017050501) {
        // Remove any duplicate rows from leeloocert issue table.
        // This SQL fetches the id of those records which have duplicate leeloocert issues.
        // This doesn't return the first issue.
        $fromclause = "FROM (
                             SELECT min(id) AS minid, userid, leeloocertid
                               FROM {leeloocert_issues}
                           GROUP BY userid, leeloocertid
                            ) minid
                       JOIN {leeloocert_issues} ci
                         ON ci.userid = minid.userid
                        AND ci.leeloocertid = minid.leeloocertid
        ...
        if ($records = $DB->get_records_sql($getduplicatessql)) {
            ...
            $DB->delete_records_select('leeloocert_issues', "id IN ($ids)");
        }

        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050501, 'leeloocert');
    }
    ...
    if ($oldversion < 2017050502) {
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050502, 'leeloocert');
    }

    if ($oldversion < 2017050506) {
        $table = new xmldb_table('leeloocert_elements');
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050506, 'leeloocert');
    }
    ...
    if ($oldversion < 2018051705) {
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2018051705, 'leeloocert');
    }
    ...
    if ($oldversion < 2019111803) {
        ...
        $key = new xmldb_key('templateid', XMLDB_KEY_FOREIGN, ['templateid'], 'leeloocert_templates', ['id']);
        $dbman->add_key($table, $key);

        $table = new xmldb_table('leeloocert_pages');
        $index = new xmldb_index('templateid', XMLDB_INDEX_UNIQUE, ['templateid']);
        if ($dbman->index_exists($table, $index)) {
            $dbman->drop_index($table, $index);
        }
        ...
        upgrade_mod_savepoint(true, 2019111803, 'leeloocert');
    }
    ...
    if ($oldversion < 2020061501) {
        ...
        upgrade_mod_savepoint(true, 2020061501, 'leeloocert');
    }
