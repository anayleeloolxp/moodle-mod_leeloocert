...
/**
 * This file contains the leeloocert date range element.
 *
 * @package    leeloocertelement_daterange
 * @copyright  2018 Dmitrii Metelkin <dmitriim@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloocertelement_daterange;

use \mod_leeloocert\element_helper;
...
/**
 * The leeloocert date range element.
 *
 * @package    leeloocertelement_daterange
 * @copyright  2018 Dmitrii Metelkin <dmitriim@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloocert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[self::DATE_ISSUE] = get_string('issueddate', 'leeloocertelement_daterange');
        $dateoptions[self::DATE_CURRENT_DATE] = get_string('currentdate', 'leeloocertelement_daterange');
        $dateoptions[self::DATE_COMPLETION] = get_string('completiondate', 'leeloocertelement_daterange');
        $dateoptions[self::DATE_COURSE_START] = get_string('coursestartdate', 'leeloocertelement_daterange');
        $dateoptions[self::DATE_COURSE_END] = get_string('courseenddate', 'leeloocertelement_daterange');
        $dateoptions[self::DATE_COURSE_GRADE] = get_string('coursegradedate', 'leeloocertelement_daterange');

        $dateoptions = $dateoptions + element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'leeloocertelement_daterange'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'leeloocertelement_daterange');

        parent::render_form_elements($mform);

        $mform->addElement('header', 'dateranges', get_string('dateranges', 'leeloocertelement_daterange'));
        $mform->addElement('static', 'help', '', get_string('help', 'leeloocertelement_daterange'));
        $mform->addElement('static', 'placeholders', '', get_string('placeholders', 'leeloocertelement_daterange'));

        $mform->addElement('text', 'fallbackstring', get_string('fallbackstring', 'leeloocertelement_daterange'));
        $mform->addHelpButton('fallbackstring', 'fallbackstring', 'leeloocertelement_daterange');
        ...
        $ranges[] = $mform->createElement(
            'date_selector',
            'startdate',
            get_string('start', 'leeloocertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'date_selector',
            'enddate',
            get_string('end', 'leeloocertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'checkbox',
            'recurring',
            get_string('recurring', 'leeloocertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'text',
            'datestring',
            get_string('datestring', 'leeloocertelement_daterange'),
            ['class' => 'datestring']
        );
        ...
        $ranges[] = $mform->createElement(
            'advcheckbox',
            'rangedelete',
            get_string('setdeleted', 'leeloocertelement_daterange'),
            '',
            [],
            [0, 1]
        );
        ...
        $rangeoptions['recurring']['helpbutton'] = ['recurring', 'leeloocertelement_daterange'];
        $rangeoptions['datestring']['type'] = PARAM_NOTAGS;
        $rangeoptions['rangedelete']['type'] = PARAM_BOOL;

        $addstring = get_string('addrange', 'leeloocertelement_daterange');
    ...
    public function validate_form_elements($data, $files) {
        ...
        $error = get_string('error:atleastone', 'leeloocertelement_daterange');
        ...
        for ($i = 0; $i < $data['repeats']; $i++) {
            ...
            if (empty($data['datestring'][$i])) {
                ...
                $errors[$name] = get_string('error:datestring', 'leeloocertelement_daterange');
            }

            // Check that end date is correctly set.
            if ( $data['startdate'][$i] >= $data['enddate'][$i] ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:enddate', 'leeloocertelement_daterange');
            }
            ...
            if (!empty($data['recurring'][$i]) && $rangeperiod >= self::MAX_RECURRING_PERIOD ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:recurring', 'leeloocertelement_daterange');
            }
        }
    ...
    public function render_html() {
        ...
        return element_helper::render_html_content($this, get_string('preview', 'leeloocertelement_daterange', $this->get_name()));
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $data->dateitem)) {
            ...
            $DB->set_field('leeloocert_elements', 'data', $this->save_unique_data($data), array('id' => $this->get_id()));
        }
    }
