...
/**
 * This file contains the leeloolxpcert date range element.
 *
 * @package    leeloolxpcertelement_daterange
 * @copyright  2018 Dmitrii Metelkin <dmitriim@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloolxpcertelement_daterange;

use \mod_leeloolxpcert\element_helper;
...
/**
 * The leeloolxpcert date range element.
 *
 * @package    leeloolxpcertelement_daterange
 * @copyright  2018 Dmitrii Metelkin <dmitriim@catalyst-au.net>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloolxpcert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[self::DATE_ISSUE] = get_string('issueddate', 'leeloolxpcertelement_daterange');
        $dateoptions[self::DATE_CURRENT_DATE] = get_string('currentdate', 'leeloolxpcertelement_daterange');
        $dateoptions[self::DATE_COMPLETION] = get_string('completiondate', 'leeloolxpcertelement_daterange');
        $dateoptions[self::DATE_COURSE_START] = get_string('coursestartdate', 'leeloolxpcertelement_daterange');
        $dateoptions[self::DATE_COURSE_END] = get_string('courseenddate', 'leeloolxpcertelement_daterange');
        $dateoptions[self::DATE_COURSE_GRADE] = get_string('coursegradedate', 'leeloolxpcertelement_daterange');

        $dateoptions = $dateoptions + element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'leeloolxpcertelement_daterange'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'leeloolxpcertelement_daterange');

        parent::render_form_elements($mform);

        $mform->addElement('header', 'dateranges', get_string('dateranges', 'leeloolxpcertelement_daterange'));
        $mform->addElement('static', 'help', '', get_string('help', 'leeloolxpcertelement_daterange'));
        $mform->addElement('static', 'placeholders', '', get_string('placeholders', 'leeloolxpcertelement_daterange'));

        $mform->addElement('text', 'fallbackstring', get_string('fallbackstring', 'leeloolxpcertelement_daterange'));
        $mform->addHelpButton('fallbackstring', 'fallbackstring', 'leeloolxpcertelement_daterange');
        ...
        $ranges[] = $mform->createElement(
            'date_selector',
            'startdate',
            get_string('start', 'leeloolxpcertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'date_selector',
            'enddate',
            get_string('end', 'leeloolxpcertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'checkbox',
            'recurring',
            get_string('recurring', 'leeloolxpcertelement_daterange')
        );

        $ranges[] = $mform->createElement(
            'text',
            'datestring',
            get_string('datestring', 'leeloolxpcertelement_daterange'),
            ['class' => 'datestring']
        );
        ...
        $ranges[] = $mform->createElement(
            'advcheckbox',
            'rangedelete',
            get_string('setdeleted', 'leeloolxpcertelement_daterange'),
            '',
            [],
            [0, 1]
        );
        ...
        $rangeoptions['recurring']['helpbutton'] = ['recurring', 'leeloolxpcertelement_daterange'];
        $rangeoptions['datestring']['type'] = PARAM_NOTAGS;
        $rangeoptions['rangedelete']['type'] = PARAM_BOOL;

        $addstring = get_string('addrange', 'leeloolxpcertelement_daterange');
    ...
    public function validate_form_elements($data, $files) {
        ...
        $error = get_string('error:atleastone', 'leeloolxpcertelement_daterange');
        ...
        for ($i = 0; $i < $data['repeats']; $i++) {
            ...
            if (empty($data['datestring'][$i])) {
                ...
                $errors[$name] = get_string('error:datestring', 'leeloolxpcertelement_daterange');
            }

            // Check that end date is correctly set.
            if ( $data['startdate'][$i] >= $data['enddate'][$i] ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:enddate', 'leeloolxpcertelement_daterange');
            }
            ...
            if (!empty($data['recurring'][$i]) && $rangeperiod >= self::MAX_RECURRING_PERIOD ) {
                $errors[$this->build_element_name('enddate', $i)] = get_string('error:recurring', 'leeloolxpcertelement_daterange');
            }
        }
    ...
    public function render($pdf, $preview, $user) {
        ...
        if ($preview) {
            ...
            // Get the leeloolxpcert this page belongs to.
            $leeloolxpcert = $DB->get_record('leeloolxpcert', array('templateid' => $page->templateid), '*', MUST_EXIST);
            // Now we can get the issue for this user.
            $issue = $DB->get_record('leeloolxpcert_issues', array('userid' => $user->id, 'leeloolxpcertid' => $leeloolxpcert->id),
                '*', MUST_EXIST);
    ...
    public function render_html() {
        ...
        return element_helper::render_html_content($this, get_string('preview', 'leeloolxpcertelement_daterange', $this->get_name()));
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $data->dateitem)) {
            ...
            $DB->set_field('leeloolxpcert_elements', 'data', $this->save_unique_data($data), array('id' => $this->get_id()));
        }
    }
