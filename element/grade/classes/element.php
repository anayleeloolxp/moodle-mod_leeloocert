...
/**
 * This file contains the leeloocert element grade's core interaction API.
 *
 * @package    leeloocertelement_grade
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloocertelement_grade;
...
/**
 * The leeloocert element grade's core interaction API.
 *
 * @package    leeloocertelement_grade
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloocert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $gradeitems = array();
        $gradeitems[CUSTOMCERT_GRADE_COURSE] = get_string('coursegrade', 'leeloocertelement_grade');
        $gradeitems = $gradeitems + \mod_leeloocert\element_helper::get_grade_items($COURSE);

        // The grade items.
        $mform->addElement('select', 'gradeitem', get_string('gradeitem', 'leeloocertelement_grade'), $gradeitems);
        $mform->addHelpButton('gradeitem', 'gradeitem', 'leeloocertelement_grade');

        // The grade format.
        $mform->addElement('select', 'gradeformat', get_string('gradeformat', 'leeloocertelement_grade'),
            self::get_grade_format_options());
        $mform->setType('gradeformat', PARAM_INT);
        $mform->addHelpButton('gradeformat', 'gradeformat', 'leeloocertelement_grade');
    ...
    public function render($pdf, $preview, $user) {
        ...
        \mod_leeloocert\element_helper::render_content($pdf, $this, $grade);
    }
    ...
    public function render_html() {
        ...
        return \mod_leeloocert\element_helper::render_html_content($this, $grade);
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $gradeinfo->gradeitem)) {
            ...
            $DB->set_field('leeloocert_elements', 'data', $this->save_unique_data($gradeinfo), array('id' => $this->get_id()));
        }
    }
    ...
    public static function get_grade_format_options() {
        $gradeformat = array();
        $gradeformat[GRADE_DISPLAY_TYPE_REAL] = get_string('gradepoints', 'leeloocertelement_grade');
        $gradeformat[GRADE_DISPLAY_TYPE_PERCENTAGE] = get_string('gradepercent', 'leeloocertelement_grade');
        $gradeformat[GRADE_DISPLAY_TYPE_LETTER] = get_string('gradeletter', 'leeloocertelement_grade');
