...
/**
 * This file contains the leeloolxpcert element grade's core interaction API.
 *
 * @package    leeloolxpcertelement_grade
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloolxpcertelement_grade;
 ...
 * Grade - Course
 */
define('LEELOOLXPCERT_GRADE_COURSE', '0');

/**
 * The leeloolxpcert element grade's core interaction API.
 *
 * @package    leeloolxpcertelement_grade
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloolxpcert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $gradeitems = array();
        $gradeitems[LEELOOLXPCERT_GRADE_COURSE] = get_string('coursegrade', 'leeloolxpcertelement_grade');
        $gradeitems = $gradeitems + \mod_leeloolxpcert\element_helper::get_grade_items($COURSE);

        // The grade items.
        $mform->addElement('select', 'gradeitem', get_string('gradeitem', 'leeloolxpcertelement_grade'), $gradeitems);
        $mform->addHelpButton('gradeitem', 'gradeitem', 'leeloolxpcertelement_grade');

        // The grade format.
        $mform->addElement(
            'select',
            'gradeformat',
            get_string('gradeformat', 'leeloolxpcertelement_grade'),
            self::get_grade_format_options()
        );
        $mform->setType('gradeformat', PARAM_INT);
        $mform->addHelpButton('gradeformat', 'gradeformat', 'leeloolxpcertelement_grade');
    ...
    public function render($pdf, $preview, $user) {
        ...
        if ($preview) {
            ...
            if ($gradeitem == LEELOOLXPCERT_GRADE_COURSE) {
        ...
        \mod_leeloolxpcert\element_helper::render_content($pdf, $this, $grade);
    }
    ...
    public function render_html() {
        ...
        return \mod_leeloolxpcert\element_helper::render_html_content($this, $grade);
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $gradeinfo->gradeitem)) {
            ...
            $DB->set_field('leeloolxpcert_elements', 'data', $this->save_unique_data($gradeinfo), array('id' => $this->get_id()));
        }
    }
    ...
    public static function get_grade_format_options() {
        $gradeformat = array();
        $gradeformat[GRADE_DISPLAY_TYPE_REAL] = get_string('gradepoints', 'leeloolxpcertelement_grade');
        $gradeformat[GRADE_DISPLAY_TYPE_PERCENTAGE] = get_string('gradepercent', 'leeloolxpcertelement_grade');
        $gradeformat[GRADE_DISPLAY_TYPE_LETTER] = get_string('gradeletter', 'leeloolxpcertelement_grade');
