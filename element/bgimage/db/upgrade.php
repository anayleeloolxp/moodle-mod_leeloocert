...
/**
 * leeloolxpcert background image element upgrade code.
 *
 ...
 */
function xmldb_leeloolxpcertelement_bgimage_upgrade($oldversion) {
    global $DB;

    if ($oldversion < 2016120501) {
        // Go through each 'image' element and update the file stored information.
        if ($images = $DB->get_records_select('leeloolxpcert_elements', $DB->sql_compare_text('element') . ' = \'bgimage\'')) {
            ...
            foreach ($images as $image) {
                ...
                if ($file = $fs->get_file_by_hash($elementinfo->pathnamehash)) {
                    ...
                    $DB->set_field('leeloolxpcert_elements', 'data', $arrtostore,  array('id' => $image->id));
                }
            }
        }

        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2016120501, 'leeloolxpcertelement', 'bgimage');
    }
