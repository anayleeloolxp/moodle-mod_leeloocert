...
/**
 * This file contains the leeloolxpcert element userpicture's core interaction API.
 *
 * @package    leeloolxpcertelement_userpicture
 * @copyright  2017 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloolxpcertelement_userpicture;

defined('MOODLE_INTERNAL') || die();
/**
 * The leeloolxpcert element userpicture's core interaction API.
 *
 * @package    leeloolxpcertelement_userpicture
 * @copyright  2017 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloolxpcert\element {
    ...
    public function render_form_elements($mform) {
        $mform->addElement('text', 'width', get_string('width', 'leeloolxpcertelement_userpicture'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'width', 'leeloolxpcertelement_userpicture');
        $mform->addElement('text', 'height', get_string('height', 'leeloolxpcertelement_userpicture'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'height', 'leeloolxpcertelement_userpicture');
    ...
    public function validate_form_elements($data, $files) {
        ...
        if ((!isset($data['width'])) || (!is_numeric($data['width'])) || ($data['width'] < 0)) {
            $errors['width'] = get_string('invalidwidth', 'leeloolxpcertelement_userpicture');
        }
        // Check if height is not set, or not numeric or less than 0.
        if ((!isset($data['height'])) || (!is_numeric($data['height'])) || ($data['height'] < 0)) {
            $errors['height'] = get_string('invalidheight', 'leeloolxpcertelement_userpicture');
        }
