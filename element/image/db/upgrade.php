...
/**
 * leeloolxpcert image element upgrade code.
 *
 ...
 */
function xmldb_leeloolxpcertelement_image_upgrade($oldversion) {
    global $DB;

    if ($oldversion < 2016120501) {
        // Go through each 'image' element and update the file stored information.
        if ($images = $DB->get_records_select('leeloolxpcert_elements', $DB->sql_compare_text('element') . ' = \'image\'')) {
        ...
        // Savepoint reached.
        upgrade_plugin_savepoint(true, 2016120501, 'leeloolxpcertelement', 'image');
    }
