 ...
 * This file contains the leeloolxpcert element date's core interaction API.

 *

 * @package    leeloolxpcertelement_date
...
namespace leeloolxpcertelement_date;
...
define('LEELOOLXPCERT_DATE_COURSE_GRADE', '0');
...
define('LEELOOLXPCERT_DATE_ISSUE', '-1');
...
define('LEELOOLXPCERT_DATE_COMPLETION', '-2');
...
define('LEELOOLXPCERT_DATE_COURSE_START', '-3');
...
define('LEELOOLXPCERT_DATE_COURSE_END', '-4');
...
define('LEELOOLXPCERT_DATE_CURRENT_DATE', '-5');
...
define('LEELOOLXPCERT_DATE_ENROLMENT_START', '-6');
...
define('LEELOOLXPCERT_DATE_ENROLMENT_END', '-7');
...
define('LEELOOLXPCERT_DATE_VALID_THRU', '-8');

require_once($CFG->dirroot . '/lib/grade/constants.php');

/**

 * The leeloolxpcert element date's core interaction API.
...
class element extends \mod_leeloolxpcert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions[LEELOOLXPCERT_DATE_ISSUE] = get_string('issueddate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_VALID_THRU] = get_string('validthrudate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_CURRENT_DATE] = get_string('currentdate', 'leeloolxpcertelement_date');

        $completionenabled = $CFG->enablecompletion && ($COURSE->id == SITEID || $COURSE->enablecompletion);

        if ($completionenabled) {

            $dateoptions[LEELOOLXPCERT_DATE_COMPLETION] = get_string('completiondate', 'leeloolxpcertelement_date');
        }

        $dateoptions[LEELOOLXPCERT_DATE_ENROLMENT_START] = get_string('enrolmentstartdate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_ENROLMENT_END] = get_string('enrolmentenddate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_COURSE_START] = get_string('coursestartdate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_COURSE_END] = get_string('courseenddate', 'leeloolxpcertelement_date');

        $dateoptions[LEELOOLXPCERT_DATE_COURSE_GRADE] = get_string('coursegradedate', 'leeloolxpcertelement_date');

        $dateoptions = $dateoptions + \mod_leeloolxpcert\element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'leeloolxpcertelement_date'), $dateoptions);

        $mform->addHelpButton('dateitem', 'dateitem', 'leeloolxpcertelement_date');

        $mform->addElement('select', 'dateformat', get_string('dateformat', 'leeloolxpcertelement_date'), self::get_date_formats());

        $mform->addHelpButton('dateformat', 'dateformat', 'leeloolxpcertelement_date');
    ...
    public function render($pdf, $preview, $user) {
        ...
        $courseid = \mod_leeloolxpcert\element_helper::get_courseid($this->id);
        ...
        if ($preview) {
            ...
            $leeloolxpcert = $DB->get_record('leeloolxpcert', array('templateid' => $page->templateid), '*', MUST_EXIST);

            if (!empty($leeloolxpcert->certificatevalidthru)) {

                $validthrumonth = $leeloolxpcert->certificatevalidthru;

                $date = strtotime("+$validthrumonth month", time());
            }
        } else {
            ...
            if ($dateitem == LEELOOLXPCERT_DATE_ISSUE) {

                $date = $issue->timecreated;
            } else if ($dateitem == LEELOOLXPCERT_DATE_VALID_THRU) {
                $date = time();
                if (!empty($user->aroptions)) {

                    if (!empty($user->aroptions->valid_thru)) {

                        $validthrumonth = $user->aroptions->valid_thru;

                        $date = strtotime("+$validthrumonth month", $issue->timecreated);
                    }
                }
            } else if ($dateitem == LEELOOLXPCERT_DATE_CURRENT_DATE) {

                $date = time();
            } else if ($dateitem == LEELOOLXPCERT_DATE_COMPLETION) {
            ...
            } else if ($dateitem == LEELOOLXPCERT_DATE_ENROLMENT_START) {
            ...
            } else if ($dateitem == LEELOOLXPCERT_DATE_ENROLMENT_END) {
            ...
            } else if ($dateitem == LEELOOLXPCERT_DATE_COURSE_START) {

                $date = $DB->get_field('course', 'startdate', array('id' => $courseid));
            } else if ($dateitem == LEELOOLXPCERT_DATE_COURSE_END) {

                $date = $DB->get_field('course', 'enddate', array('id' => $courseid));
            } else {

                if ($dateitem == LEELOOLXPCERT_DATE_COURSE_GRADE) {
        ...
        if (!empty($date)) {
            ...
            \mod_leeloolxpcert\element_helper::render_content($pdf, $this, $this->get_date_format_string($date, $dateformat));
        }
    }
    ...
    public function render_html() {
        ...
        return \mod_leeloolxpcert\element_helper::render_html_content($this, $this->get_date_format_string(time(), $dateformat));
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $dateinfo->dateitem)) {
            ...
            $DB->set_field('leeloolxpcert_elements', 'data', $this->save_unique_data($dateinfo), array('id' => $this->get_id()));
        }
    }
        ...
        if (!in_array(($day % 100), array(11, 12, 13))) {
            ...
            switch ($day % 10) {
                ...
                case 1:
                    ...
                    return get_string('numbersuffix_st_as_in_first', 'leeloolxpcertelement_date');

                case 2:

                    return get_string('numbersuffix_nd_as_in_second', 'leeloolxpcertelement_date');

                case 3:

                    return get_string('numbersuffix_rd_as_in_third', 'leeloolxpcertelement_date');
            }
        }
