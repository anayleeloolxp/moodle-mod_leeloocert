...
/**
 * This file contains the leeloocert element date's core interaction API.
 *
 * @package    leeloocertelement_date
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace leeloocertelement_date;
 ...
 * Date - Validate thru
 */
define('CUSTOMCERT_DATE_VALID_THRU', '-8');

require_once($CFG->dirroot . '/lib/grade/constants.php');

/**
 * The leeloocert element date's core interaction API.
 *
 * @package    leeloocertelement_date
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element extends \mod_leeloocert\element {
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[CUSTOMCERT_DATE_ISSUE] = get_string('issueddate', 'leeloocertelement_date');
        $dateoptions[CUSTOMCERT_DATE_VALID_THRU] = get_string('validthrudate', 'leeloocertelement_date');
        $dateoptions[CUSTOMCERT_DATE_CURRENT_DATE] = get_string('currentdate', 'leeloocertelement_date');
        $completionenabled = $CFG->enablecompletion && ($COURSE->id == SITEID || $COURSE->enablecompletion);
        if ($completionenabled) {
            $dateoptions[CUSTOMCERT_DATE_COMPLETION] = get_string('completiondate', 'leeloocertelement_date');
        }
        $dateoptions[CUSTOMCERT_DATE_ENROLMENT_START] = get_string('enrolmentstartdate', 'leeloocertelement_date');
        $dateoptions[CUSTOMCERT_DATE_ENROLMENT_END] = get_string('enrolmentenddate', 'leeloocertelement_date');

        $dateoptions[CUSTOMCERT_DATE_COURSE_START] = get_string('coursestartdate', 'leeloocertelement_date');
        $dateoptions[CUSTOMCERT_DATE_COURSE_END] = get_string('courseenddate', 'leeloocertelement_date');
        $dateoptions[CUSTOMCERT_DATE_COURSE_GRADE] = get_string('coursegradedate', 'leeloocertelement_date');
        $dateoptions = $dateoptions + \mod_leeloocert\element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'leeloocertelement_date'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'leeloocertelement_date');

        $mform->addElement('select', 'dateformat', get_string('dateformat', 'leeloocertelement_date'), self::get_date_formats());
        $mform->addHelpButton('dateformat', 'dateformat', 'leeloocertelement_date');
    ...
    public function render($pdf, $preview, $user) {
        ...
        if ($preview) {
            ...
            if (!empty($leeloocert->certificatevalidthru)) {
                $validthrumonth = $leeloocert->certificatevalidthru;
            ...
            $issue = $DB->get_record(
                'leeloocert_issues',
                array('userid' => $user->id, 'leeloocertid' => $leeloocert->id),
                '*',
                IGNORE_MULTIPLE
            );
            ...
            if ($dateitem == CUSTOMCERT_DATE_ISSUE) {
            ...
            } else if ($dateitem == CUSTOMCERT_DATE_VALID_THRU) {
                $date = time();
                if (!empty($leeloocert->certificatevalidthru)) {
                    $validthrumonth = $leeloocert->certificatevalidthru;
                    $date = strtotime("+$validthrumonth month", time());
                ...
                if ($dateitem == CUSTOMCERT_DATE_COURSE_GRADE) {
                    $grade = \mod_leeloocert\element_helper::get_course_grade_info(
                        $courseid,
                    ...
                    $grade = \mod_leeloocert\element_helper::get_grade_item_info(
                        $gradeitemid,
                        $dateitem,
                        $user->id
                    );
                } else {
                    $grade = \mod_leeloocert\element_helper::get_mod_grade_info(
                        $dateitem,
                        GRADE_DISPLAY_TYPE_DEFAULT,
                        $user->id
                    );
        ...
        if (!empty($date)) {
            \mod_leeloocert\element_helper::render_content($pdf, $this, $this->get_date_format_string($date, $dateformat));
        }
    }
    ...
    public function render_html() {
        ...
        return \mod_leeloocert\element_helper::render_html_content($this, $this->get_date_format_string(time(), $dateformat));
    }
    ...
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $dateinfo->dateitem)) {
            ...
            $DB->set_field('leeloocert_elements', 'data', $this->save_unique_data($dateinfo), array('id' => $this->get_id()));
        }
    }
    ...
    protected static function get_ordinal_number_suffix($day) {
        if (!in_array(($day % 100), array(11, 12, 13))) {
            switch ($day % 10) {
                    // Handle 1st, 2nd, 3rd.
                case 1:
                    return get_string('numbersuffix_st_as_in_first', 'leeloocertelement_date');
                case 2:
                    return get_string('numbersuffix_nd_as_in_second', 'leeloocertelement_date');
                case 3:
                    return get_string('numbersuffix_rd_as_in_third', 'leeloocertelement_date');
            }
        }
        return 'th';
    }
