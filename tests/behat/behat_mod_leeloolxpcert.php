...
/**
 * Contains the class responsible for step definitions related to mod_leeloolxpcert.
 *
...
/**
 * The class responsible for step definitions related to mod_leeloolxpcert.
 *
 * @package mod_leeloolxpcert
 * @category test
 * @copyright 2017 Mark Nelson <markn@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_leeloolxpcert extends behat_base {
    ...
    public function i_add_the_element_to_the_certificate_template_page($elementname, $pagenum, $templatename) {
        ...
        $template = $DB->get_record('leeloolxpcert_templates', array('name' => $templatename), '*', MUST_EXIST);
        $page = $DB->get_record(
            'leeloolxpcert_pages',
            array('templateid' => $template->id, 'sequence' => $pagenum),
            '*',
            MUST_EXIST
        );

        $this->execute('behat_forms::i_set_the_field_to', array(
            $this->escape('element_' . $page->id),
            $this->escape($elementname)
        ));
        $this->execute('behat_forms::press_button', get_string('addelement', 'leeloolxpcert'));
    }
    ...
    public function i_verify_the_custom_certificate_for_user($certificatename, $username) {
        ...
        $certificate = $DB->get_record('leeloolxpcert', array('name' => $certificatename), '*', MUST_EXIST);
        $user = $DB->get_record('user', array('username' => $username), '*', MUST_EXIST);
        $issue = $DB->get_record(
            'leeloolxpcert_issues',
            array('userid' => $user->id, 'leeloolxpcertid' => $certificate->id),
            '*',
            MUST_EXIST
        );

        $this->execute('behat_forms::i_set_the_field_to', array(get_string('code', 'leeloolxpcert'), $issue->code));
        $this->execute('behat_forms::press_button', get_string('verify', 'leeloolxpcert'));
        $this->execute('behat_general::assert_page_contains_text', get_string('verified', 'leeloolxpcert'));
        $this->execute('behat_general::assert_page_not_contains_text', get_string('notverified', 'leeloolxpcert'));
    }
    ...
    public function i_can_not_verify_the_custom_certificate_for_user($certificatename, $username) {
        ...
        $this->execute('behat_forms::i_set_the_field_to', array(get_string('code', 'leeloolxpcert'), $issue->code));
        $this->execute('behat_forms::press_button', get_string('verify', 'leeloolxpcert'));
        $this->execute('behat_general::assert_page_contains_text', get_string('notverified', 'leeloolxpcert'));
        $this->execute('behat_general::assert_page_not_contains_text', get_string('verified', 'leeloolxpcert'));
    }
    ...
    public function i_visit_the_verification_url_for_custom_certificate($certificatename) {
        ...
        $certificate = $DB->get_record('leeloolxpcert', array('name' => $certificatename), '*', MUST_EXIST);
        $template = $DB->get_record('leeloolxpcert_templates', array('id' => $certificate->templateid), '*', MUST_EXIST);

        $url = new moodle_url('/mod/leeloolxpcert/verify_certificate.php', array('contextid' => $template->contextid));
    ...
    public function i_visit_the_verification_url_for_the_site() {
        $url = new moodle_url('/mod/leeloolxpcert/verify_certificate.php');
        $this->getSession()->visit($this->locate_path($url->out_as_local_url()));
    }
}
