 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_leeloocert_element_helper_testcase extends advanced_testcase {
    ...
    /**
     * Tests we are returning the correct course id for an element in a course leeloocert activity.
     */
    public function test_get_courseid_element_in_course_certificate() {
        ...
        // Create a leeloo certificate in the course.
        $leeloocert = $this->getDataGenerator()->create_module('leeloocert', array('course' => $course->id));

        // Get the template to add elements to.
        $template = $DB->get_record('leeloocert_templates', array('contextid' => context_module::instance($leeloocert->cmid)->id));
        ...
        $element->sequence = \mod_leeloocert\element_helper::get_element_sequence($element->pageid);
        $element->timecreated = time();
        $element->id = $DB->insert_record('leeloocert_elements', $element);

        // Confirm the correct course id is returned.
        $this->assertEquals($course->id, \mod_leeloocert\element_helper::get_courseid($element->id));
    }
    ...
    public function test_get_courseid_element_in_site_template() {
        ...
        $template = \mod_leeloocert\template::create('Site template', context_system::instance()->id);
        ...
        // Confirm the correct course id is returned.
        $this->assertEquals($SITE->id, \mod_leeloocert\element_helper::get_courseid($element->id));
    }

    /**
     * Tests we are returning the correct course module id for an element in a course leeloocert activity.
     */
    public function test_get_context_element_in_course_certificate() {
        ...
        $this->assertEquals(
            context_module::instance($leeloocert->cmid),
            \mod_leeloocert\element_helper::get_context($element->id)
        );
    }
    ...
    public function test_get_context_element_in_site_template() {
        ...
        // Confirm the correct course module id is returned.
        $this->assertEquals(context_system::instance(), \mod_leeloocert\element_helper::get_context($element->id));
    }
    ...
    public function test_get_grade_items() {
        ...
        $gradeitems = \mod_leeloocert\element_helper::get_grade_items($course);
