 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_leeloocert_task_email_certificate_task_testcase extends advanced_testcase {
    ...
    public function test_email_certificates_no_elements() {
        ...
        // Create a leeloo certificate with no elements.
        $this->getDataGenerator()->create_module('leeloocert', ['course' => $course->id, 'emailstudents' => 1]);
        ...
        $task = new \mod_leeloocert\task\email_certificate_task();
        $task->execute();
    ...
    public function test_email_certificates_no_cap() {
        ...
        unassign_capability('mod/leeloocert:receiveissue', $roleids['student']);

        // Create a leeloo certificate.
        $leeloocert = $this->getDataGenerator()->create_module('leeloocert', ['course' => $course->id, 'emailstudents' => 1]);
        ...
        $element->name = 'Image';
        $DB->insert_record('leeloocert_elements', $element);
    ...
    public function test_email_certificates_students() {
        ...
        // Create a leeloo certificate.
        $leeloocert = $this->getDataGenerator()->create_module('leeloocert', array(
            'course' => $course->id,
            'emailstudents' => 1
        ));
        ...
        \mod_leeloocert\certificate::issue_certificate($leeloocert->id, $user1->id);

        // Confirm there is only one entry in this table.
        $this->assertEquals(1, $DB->count_records('leeloocert_issues'));
        ...
        $issues = $DB->get_records('leeloocert_issues');
    ...
    public function test_email_certificates_others() {
        ...
        $leeloocert = $this->getDataGenerator()->create_module('leeloocert', array(
            'course' => $course->id,
            'emailothers' => 'testleeloocert@example.com, doo@dah'
        ));
        ...
        $this->assertEquals($CFG->noreplyaddress, $emails[0]->from);
        $this->assertEquals('testleeloocert@example.com', $emails[0]->to);

        $this->assertEquals($CFG->noreplyaddress, $emails[1]->from);
        $this->assertEquals('testleeloocert@example.com', $emails[1]->to);
    }
    ...
    public function test_email_certificates_students_not_visible() {
        ...
        assign_capability('mod/leeloocert:view', CAP_PROHIBIT, $roleids['student'], \context_course::instance($course->id));
    ...
    public function test_email_certificates_students_havent_met_required_time() {
        ...
        // Create a leeloo certificate.
        $leeloocert = $this->getDataGenerator()->create_module('leeloocert', array(
            'course' => $course->id, 'emailstudents' => 1,
            'requiredtime' => '60'
        ));
