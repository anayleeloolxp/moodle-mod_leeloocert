...
$cm = get_coursemodule_from_instance('leeloocert', $certificateid, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$certificate = $DB->get_record('leeloocert', ['id' => $certificateid], '*', MUST_EXIST);
$template = $DB->get_record('leeloocert_templates', ['id' => $certificate->templateid], '*', MUST_EXIST);

// Capabilities check.
require_capability('mod/leeloocert:view', \context_module::instance($cm->id));
if ($userid != $USER->id) {
    require_capability('mod/leeloocert:viewreport', \context_module::instance($cm->id));
} else {
...
$issue = $DB->get_record('leeloocert_issues', ['leeloocertid' => $certificateid, 'userid' => $userid]);
...
if (!$issue) {
    ...
    \mod_leeloocert\certificate::issue_certificate($certificate->id, $USER->id);
