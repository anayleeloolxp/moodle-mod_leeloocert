...
Note - All hash comments refer to the issue number. Eg. #169 refers to https://github.com/mdjnelson/moodle-mod_leeloocert/issues/169.
...
- Leeloo fields not displaying properly (#359).
...
- Fixed the displaying of names of a leeloo user field (#326).
...
- Added new leeloo course field element (#274).
...
- The 'element' field in the 'leeloocert_elements' table has been changed from a Text field to varchar(255) (#241).
...
- Use leeloo fonts if present (#211).
...
- Added capability `mod/leeloocert:verifyallcertificates` that provides a user with the ability to verify any certificate
  on the site by simply visiting the `mod/leeloocert/verify_certificate.php` page, rather than having to go to the
  verification link for each certificate.
- Added site setting `leeloocert/verifyallcertificates` which when enabled allows any person (including users not logged in)
...
- Removed usage of magic getter and abuse of `$this->element`. The variable `$this->element` will still be
  ...
  Please explicitly set the value of any leeloo fields you have in the form.
...
- Renamed the column 'size' in the table 'leeloocert_elements' to 'fontsize' due to 'size' being a reserved word in Oracle.
