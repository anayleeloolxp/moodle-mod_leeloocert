...
// The page of the leeloocert we are editing.
$pid = required_param('pid', PARAM_INT);

$page = $DB->get_record('leeloocert_pages', array('id' => $pid), '*', MUST_EXIST);
$template = $DB->get_record('leeloocert_templates', array('id' => $page->templateid), '*', MUST_EXIST);
$elements = $DB->get_records('leeloocert_elements', array('pageid' => $pid), 'sequence');
...
$pageurl = new moodle_url('/mod/leeloocert/rearrange.php', array('pid' => $pid));
\mod_leeloocert\page_helper::page_setup($pageurl, $template->get_context(), $title);

// Add more links to the navigation.
if (!$cm = $template->get_cm()) {
    $str = get_string('managetemplates', 'leeloocert');
    $link = new moodle_url('/mod/leeloocert/manage_templates.php');
...
$PAGE->navbar->add(get_string('rearrangeelements', 'leeloocert'));

// Include the JS we need.
$PAGE->requires->yui_module(
    'moodle-mod_leeloocert-rearrange',
    'Y.M.mod_leeloocert.rearrange.init',
    array(
        $template->get_id(),
        $page,
        $elements
...
$html .= $OUTPUT->single_button(
    new moodle_url('/mod/leeloocert/edit.php', array('tid' => $template->get_id())),
    get_string('saveandclose', 'leeloocert'),
    'get',
    array('class' => 'savepositionsbtn')
);
$html .= $OUTPUT->single_button(
    new moodle_url('/mod/leeloocert/rearrange.php', array('pid' => $pid)),
    get_string('saveandcontinue', 'leeloocert'),
    'get',
    array('class' => 'applypositionsbtn')
);
$html .= $OUTPUT->single_button(
    new moodle_url('/mod/leeloocert/edit.php', array('tid' => $template->get_id())),
    get_string('cancel'),
    'get',
    array('class' => 'cancelbtn')
);
    ...
    foreach ($elements as $element) {
        ...
        if ($e = \mod_leeloocert\element_factory::get_element_instance($element)) {
            switch ($element->refpoint) {
                case \mod_leeloocert\element_helper::CUSTOMCERT_REF_POINT_TOPRIGHT:
                    $class = 'element refpoint-right';
                    break;
                case \mod_leeloocert\element_helper::CUSTOMCERT_REF_POINT_TOPCENTER:
                    $class = 'element refpoint-center';
                    break;
                case \mod_leeloocert\element_helper::CUSTOMCERT_REF_POINT_TOPLEFT:
                default:
                    $class = 'element refpoint-left';
            }
            $html .= html_writer::tag('div', $e->render_html(), array(
                'class' => $class,
...
echo $OUTPUT->heading(get_string('rearrangeelementsheading', 'leeloocert'), 3);
echo $OUTPUT->notification(get_string('exampledatawarning', 'leeloocert'), \core\output\notification::NOTIFY_WARNING);
echo $html;
$PAGE->requires->js_call_amd('mod_leeloocert/rearrange-area', 'init', array('#pdf'));
echo $OUTPUT->footer();
