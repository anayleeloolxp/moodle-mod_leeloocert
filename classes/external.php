...
class external extends \external_api {
    ...
    public static function save_element($templateid, $elementid, $values) {
        ...
        $template = $DB->get_record('leeloocert_templates', array('id' => $templateid), '*', MUST_EXIST);
    ...
    /**
     * Handles deleting a leeloocert issue.
     *
    ...
    public static function delete_issue($certificateid, $issueid) {
        ...
        $issue = $DB->get_record('leeloocert_issues', ['id' => $issueid, 'leeloocertid' => $certificateid], '*', MUST_EXIST);

        $cm = get_coursemodule_from_instance('leeloocert', $certificate->id, 0, false, MUST_EXIST);
        ...
        // Delete the issue.
        return $DB->delete_records('leeloocert_issues', ['id' => $issue->id]);
    }
