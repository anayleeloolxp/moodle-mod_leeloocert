...
/**
 * This file contains the form for handling the layout of the leeloocert instance.
 *
...
require_once($CFG->dirroot . '/mod/leeloocert/includes/colourpicker.php');

\MoodleQuickForm::registerElementType('leeloocert_colourpicker',
    $CFG->dirroot . '/mod/leeloocert/includes/colourpicker.php', 'MoodleQuickForm_leeloocert_colourpicker');

/**
 * The form for handling the layout of the leeloocert instance.
 *
 * @package    mod_leeloocert
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class edit_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('name', 'leeloocert'), 'maxlength="255"');
        ...
        if (isset($this->_customdata['tid'])) {
            ...
            if ($pages = $DB->get_records('leeloocert_pages', array('templateid' => $this->tid), 'sequence')) {
                $this->numpages = count($pages);
                foreach ($pages as $p) {
                    $this->add_leeloocert_page_elements($p);
                }
            }
            ...
            $page->id = 0;
            $page->sequence = 1;
            $this->add_leeloocert_page_elements($page);
        }

        // Link to add another page, only display it when the template has been created.
        if (isset($this->_customdata['tid'])) {
            $addpagelink = new \moodle_url('/mod/leeloocert/edit.php',
                array(
                    'tid' => $this->tid,
                    'aid' => 1,
                    'action' => 'addpage',
            ...
            $icon = $OUTPUT->pix_icon('t/switch_plus', get_string('addcertpage', 'leeloocert'));
            $addpagehtml = \html_writer::link($addpagelink, $icon . get_string('addcertpage', 'leeloocert'));
        ...
        $group[] = $mform->createElement('submit', 'previewbtn', get_string('savechangespreview', 'leeloocert'), array(), false);
    ...
    /**
     * Fill in the current page data for this leeloocert.
     */
    public function definition_after_data() {
        ...
        // Check that we are updating a current leeloocert.
        if ($this->tid) {
            // Get the pages for this leeloocert.
            if ($pages = $DB->get_records('leeloocert_pages', array('templateid' => $this->tid))) {
    ...
    public function validation($data, $files) {
        ...
        if (\core_text::strlen($data['name']) > 255) {
            $errors['name'] = get_string('nametoolong', 'leeloocert');
        }
        ...
        foreach ($data as $key => $value) {
            if (strpos($key, 'pagewidth_') !== false) {
                ...
                if ((!isset($data[$widthid])) || (!is_numeric($data[$widthid])) || ($data[$widthid] <= 0)) {
                    $errors[$widthid] = get_string('invalidwidth', 'leeloocert');
                }
            }
            if (strpos($key, 'pageheight_') !== false) {
                ...
                if ((!isset($data[$heightid])) || (!is_numeric($data[$heightid])) || ($data[$heightid] <= 0)) {
                    $errors[$heightid] = get_string('invalidheight', 'leeloocert');
                }
            }
            if (strpos($key, 'pageleftmargin_') !== false) {
                // Validate that the left margin is a valid value.
                if (isset($data[$key]) && ($data[$key] < 0)) {
                    $errors[$key] = get_string('invalidmargin', 'leeloocert');
                }
            }
     ...
     *
     * @param \stdClass $page the leeloocert page
     */
    protected function add_leeloocert_page_elements($page) {
        global $DB, $OUTPUT;
        ...
        if ($this->numpages > 1) {
            $mform->addElement('header', 'page_' . $page->id, get_string('page', 'leeloocert', $page->sequence));
        }

        $editlink = '/mod/leeloocert/edit.php';
        $editlinkparams = array('tid' => $this->tid, 'sesskey' => sesskey());
        $editelementlink = '/mod/leeloocert/edit_element.php';
        ...
        $mform->addElement('text', 'pagewidth_' . $page->id, get_string('width', 'leeloocert'));
        ...
        $mform->addHelpButton('pagewidth_' . $page->id, 'width', 'leeloocert');

        $mform->addElement('text', 'pageheight_' . $page->id, get_string('height', 'leeloocert'));
        ...
        $mform->addHelpButton('pageheight_' . $page->id, 'height', 'leeloocert');

        $mform->addElement('text', 'pageleftmargin_' . $page->id, get_string('leftmargin', 'leeloocert'));
        $mform->setType('pageleftmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pageleftmargin_' . $page->id, 0);
        $mform->addHelpButton('pageleftmargin_' . $page->id, 'leftmargin', 'leeloocert');

        $mform->addElement('text', 'pagerightmargin_' . $page->id, get_string('rightmargin', 'leeloocert'));
        $mform->setType('pagerightmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pagerightmargin_' . $page->id, 0);
        $mform->addHelpButton('pagerightmargin_' . $page->id, 'rightmargin', 'leeloocert');

        // Check if there are elements to add.
        if ($elements = $DB->get_records('leeloocert_elements', array('pageid' => $page->id), 'sequence ASC')) {
            // Get the total number of elements.
            $numelements = count($elements);
            // Create a table to display these elements.
            $table = new \html_table();
            $table->attributes = array('class' => 'generaltable elementstable');
            $table->head  = array(get_string('name', 'leeloocert'), get_string('type', 'leeloocert'), get_string('actions'));
            $table->align = array('left', 'left', 'left');
            // Loop through and add the elements to the table.
            foreach ($elements as $element) {
                $elementname = new \core\output\inplace_editable('mod_leeloocert', 'elementname', $element->id,
            ...
            $link = \html_writer::link(new \moodle_url('/mod/leeloocert/rearrange.php', array('pid' => $page->id)),
                get_string('rearrangeelements', 'leeloocert'));
            // Add the table to the form.
            $mform->addElement('static', 'elements_' . $page->id, get_string('elements', 'leeloocert'), \html_writer::table($table)
                . \html_writer::tag( 'div', $link));
            $mform->addHelpButton('elements_' . $page->id, 'elements', 'leeloocert');
        }

        $group = array();
        $group[] = $mform->createElement('select', 'element_' . $page->id, '', element_helper::get_available_element_types());
        $group[] = $mform->createElement('submit', 'addelement_' . $page->id, get_string('addelement', 'leeloocert'),
            array(), false);
        ...
        if ($this->numpages > 1) {
            ...
            $icon = $OUTPUT->pix_icon('t/delete', get_string('deletecertpage', 'leeloocert'));
            $deletepagehtml = \html_writer::link($deletelink, $icon . get_string('deletecertpage', 'leeloocert'));
