...
/**
 * This file contains the form for handling the layout of the leeloolxpcert instance.
 *
...
/**
 * The form for handling the layout of the leeloolxpcert instance.
 *
 * @package    mod_leeloolxpcert
 * @copyright  2013 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class edit_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('name', 'leeloolxpcert'), 'maxlength="255"');
        ...
        if (isset($this->_customdata['tid'])) {
            ...
            if ($pages = $DB->get_records('leeloolxpcert_pages', array('templateid' => $this->tid), 'sequence')) {
                $this->numpages = count($pages);
                foreach ($pages as $p) {
                    $this->add_leeloolxpcert_page_elements($p);
                }
            }
        ...
        if (isset($this->_customdata['tid'])) {
            ...
            $icon = $OUTPUT->pix_icon('t/switch_plus', get_string('addcertpage', 'leeloolxpcert'));
            $addpagehtml = \html_writer::link($addpagelink, $icon . get_string('addcertpage', 'leeloolxpcert'));
        ...
        $group[] = $mform->createElement('submit', 'previewbtn', get_string('savechangespreview', 'leeloolxpcert'), array(), false);
    ...
    public function definition_after_data() {
        ...
        // Check that we are updating a current leeloolxpcert.
        if ($this->tid) {
            // Get the pages for this leeloolxpcert.
            if ($pages = $DB->get_records('leeloolxpcert_pages', array('templateid' => $this->tid))) {
        ...
        foreach ($data as $key => $value) {
            if (strpos($key, 'pagewidth_') !== false) {
                ...
                if ((!isset($data[$widthid])) || (!is_numeric($data[$widthid])) || ($data[$widthid] <= 0)) {
                    $errors[$widthid] = get_string('invalidwidth', 'leeloolxpcert');
                }
            }
            if (strpos($key, 'pageheight_') !== false) {
                ...
                if ((!isset($data[$heightid])) || (!is_numeric($data[$heightid])) || ($data[$heightid] <= 0)) {
                    $errors[$heightid] = get_string('invalidheight', 'leeloolxpcert');
                }
            }
            if (strpos($key, 'pageleftmargin_') !== false) {
                // Validate that the left margin is a valid value.
                if (isset($data[$key]) && ($data[$key] < 0)) {
                    $errors[$key] = get_string('invalidmargin', 'leeloolxpcert');
                }
            }
     ...
     *
     * @param \stdClass $page the leeloolxpcert page
     */
    protected function add_leeloolxpcert_page_elements($page) {
        global $DB, $OUTPUT;
        ...
        if ($this->numpages > 1) {
            $mform->addElement('header', 'page_' . $page->id, get_string('page', 'leeloolxpcert', $page->sequence));
        }

        $editlink = '/mod/leeloolxpcert/edit.php';
        $editlinkparams = array('tid' => $this->tid, 'sesskey' => sesskey());
        $editelementlink = '/mod/leeloolxpcert/edit_element.php';
        ...
        $mform->addElement('text', 'pagewidth_' . $page->id, get_string('width', 'leeloolxpcert'));
        ...
        $mform->addHelpButton('pagewidth_' . $page->id, 'width', 'leeloolxpcert');

        $mform->addElement('text', 'pageheight_' . $page->id, get_string('height', 'leeloolxpcert'));
        ...
        $mform->addHelpButton('pageheight_' . $page->id, 'height', 'leeloolxpcert');

        $mform->addElement('text', 'pageleftmargin_' . $page->id, get_string('leftmargin', 'leeloolxpcert'));
        $mform->setType('pageleftmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pageleftmargin_' . $page->id, 0);
        $mform->addHelpButton('pageleftmargin_' . $page->id, 'leftmargin', 'leeloolxpcert');

        $mform->addElement('text', 'pagerightmargin_' . $page->id, get_string('rightmargin', 'leeloolxpcert'));
        $mform->setType('pagerightmargin_' . $page->id, PARAM_INT);
        $mform->setDefault('pagerightmargin_' . $page->id, 0);
        $mform->addHelpButton('pagerightmargin_' . $page->id, 'rightmargin', 'leeloolxpcert');

        // Check if there are elements to add.
        if ($elements = $DB->get_records('leeloolxpcert_elements', array('pageid' => $page->id), 'sequence ASC')) {
            // Get the total number of elements.
            $numelements = count($elements);
            // Create a table to display these elements.
            $table = new \html_table();
            $table->attributes = array('class' => 'generaltable elementstable');
            $table->head  = array(get_string('name', 'leeloolxpcert'), get_string('type', 'leeloolxpcert'), get_string('actions'));
            ...
            foreach ($elements as $element) {
                ...
                $link = new \moodle_url($editelementlink, $editelementlinkparams + array(
                    'id' => $element->id,
                    'action' => 'edit'
                ));
                ...
                $link = new \moodle_url($editlink, $editlinkparams + array(
                ...
                if ($numelements > 1) {
                    ...
                    if ($element->sequence > 1) {
                        $url = new \moodle_url($editlink, $editlinkparams + array(
                            'action' => 'emoveup',
                            'aid' => $element->id
                        ));
            ...
            $link = \html_writer::link(
                new \moodle_url('/mod/leeloolxpcert/rearrange.php', array('pid' => $page->id)),
                get_string('rearrangeelements', 'leeloolxpcert')
            );
            // Add the table to the form.
            $mform->addElement('static', 'elements_' . $page->id, get_string('elements', 'leeloolxpcert'), \html_writer::table($table)
                . \html_writer::tag('div', $link));
            $mform->addHelpButton('elements_' . $page->id, 'elements', 'leeloolxpcert');
        }
        ...
        $group[] = $mform->createElement(
            'submit',
            'addelement_' . $page->id,
            get_string('addelement', 'leeloolxpcert'),
            array(),
            false
        );
        ...
        if ($this->numpages > 1) {
            ...
            $icon = $OUTPUT->pix_icon('t/delete', get_string('deletecertpage', 'leeloolxpcert'));
            $deletepagehtml = \html_writer::link($deletelink, $icon . get_string('deletecertpage', 'leeloolxpcert'));
