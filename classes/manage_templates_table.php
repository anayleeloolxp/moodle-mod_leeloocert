...
class manage_templates_table extends \table_sql {
    ...
    public function __construct($context) {
        parent::__construct('mod_leeloocert_manage_templates_table');
    ...
    public function col_actions($template) {
        ...
        $editlink = new \moodle_url('/mod/leeloocert/edit.php', array('tid' => $template->id));
        $editicon = $OUTPUT->action_icon($editlink, new \pix_icon('t/edit', get_string('edit')));

        // Link to duplicate the template.
        $duplicatelink = new \moodle_url('/mod/leeloocert/manage_templates.php',
            array(
                'tid' => $template->id,
                'action' => 'duplicate',
                'sesskey' => sesskey()
        ...
        $deletelink = new \moodle_url('/mod/leeloocert/manage_templates.php',
            array(
                'tid' => $template->id,
                'action' => 'delete',
                'sesskey' => sesskey()
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $total = $DB->count_records('leeloocert_templates', array('contextid' => $this->context->id));

        $this->pagesize($pagesize, $total);

        $this->rawdata = $DB->get_records('leeloocert_templates', array('contextid' => $this->context->id),
