...
namespace mod_leeloolxpcert\output;
...
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('leeloolxpcert', $cmid);
        $context = \context_module::instance($cm->id);
        self::require_capability($cm, $context, 'mod/leeloolxpcert:view');

        // Set some variables we are going to be using.
        $certificate = $DB->get_record('leeloolxpcert', ['id' => $cm->instance], '*', MUST_EXIST);
        ...
        if ($issues = $DB->get_records(
            ...
            ['userid' => $USER->id, 'leeloolxpcertid' => $certificate->id],
            'timecreated DESC'
        )) {
            $issue = reset($issues);
        }
        ...
        if ($certificate->requiredtime && !$canmanage) {
            if (\mod_leeloolxpcert\certificate::get_course_time($certificate->course) < ($certificate->requiredtime * 60)) {
        ...
        $fileurl = "";
        if ($requiredtimemet) {
            $fileurl = new \moodle_url('/mod/leeloolxpcert/mobile/pluginfile.php', [
        ...
        $groups = [];
        $recipients = [];
        if (has_capability('mod/leeloolxpcert:viewreport', $context)) {
            $showreport = true;
            ...
            $recipients = \mod_leeloolxpcert\certificate::get_issues($certificate->id, $groupmode, $cm, 0, 0);
            foreach ($recipients as $recipient) {
                $recipient->displayname = fullname($recipient);
                $recipient->fileurl = new \moodle_url('/mod/leeloolxpcert/mobile/pluginfile.php', [
                    'certificateid' => $certificate->id,
                    'userid' => $recipient->id
                ]);
            }
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_leeloolxpcert/mobile_view_activity_page', $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => ''
