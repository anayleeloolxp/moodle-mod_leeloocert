...
/**
 * Contains the mobile output class for the leeloo certificate.
 *
...
namespace mod_leeloocert\output;

defined('MOODLE_INTERNAL') || die();

/**
 * Mobile output class for the leeloo certificate.
 *
 * @copyright  2018 Mark Nelson <markn@moodle.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('leeloocert', $cmid);
        $context = \context_module::instance($cm->id);
        self::require_capability($cm, $context, 'mod/leeloocert:view');

        // Set some variables we are going to be using.
        $certificate = $DB->get_record('leeloocert', ['id' => $cm->instance], '*', MUST_EXIST);
        $certificate->name = format_string($certificate->name);
        list($certificate->intro, $certificate->introformat) = external_format_text(
            $certificate->intro,
        ...
        $issue = false;
        if ($issues = $DB->get_records(
            'leeloocert_issues',
            ['userid' => $USER->id, 'leeloocertid' => $certificate->id],
            'timecreated DESC'
        )) {
            $issue = reset($issues);
        }
        ...
        if ($certificate->requiredtime && !$canmanage) {
            if (\mod_leeloocert\certificate::get_course_time($certificate->course) < ($certificate->requiredtime * 60)) {
        ...
        $fileurl = "";
        if ($requiredtimemet) {
            $fileurl = new \moodle_url('/mod/leeloocert/mobile/pluginfile.php', [
        ...
        $groups = [];
        $recipients = [];
        if (has_capability('mod/leeloocert:viewreport', $context)) {
            $showreport = true;
            ...
            $recipients = \mod_leeloocert\certificate::get_issues($certificate->id, $groupmode, $cm, 0, 0);
            foreach ($recipients as $recipient) {
                $recipient->displayname = fullname($recipient);
                $recipient->fileurl = new \moodle_url('/mod/leeloocert/mobile/pluginfile.php', [
                    'certificateid' => $certificate->id,
                    'userid' => $recipient->id
                ]);
            }
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_leeloocert/mobile_view_activity_page', $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => ''
