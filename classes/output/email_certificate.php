...
class email_certificate implements \renderable, \templatable {
    ...
    public function export_for_template(\renderer_base $renderer) {
        ...
        if ($this->isstudent) {
            $data->emailgreeting = get_string('emailstudentgreeting', 'leeloolxpcert', $this->userfullname);
            $data->emailbody = get_string('emailstudentbody', 'leeloolxpcert', $info);
            $data->emailbodyplaintext = get_string('emailstudentbodyplaintext', 'leeloolxpcert', $info);
            $data->emailcertificatelink = new \moodle_url('/mod/leeloolxpcert/view.php', array('id' => $this->cmid));
            $data->emailcertificatetext = get_string('emailstudentcertificatelinktext', 'leeloolxpcert');
        } else {
            $data->emailgreeting = get_string('emailnonstudentgreeting', 'leeloolxpcert');
            $data->emailbody = get_string('emailnonstudentbody', 'leeloolxpcert', $info);
            $data->emailbodyplaintext = get_string('emailnonstudentbodyplaintext', 'leeloolxpcert', $info);
            $data->emailcertificatelink = new \moodle_url('/mod/leeloolxpcert/view.php', array('id' => $this->cmid));
            $data->emailcertificatetext = get_string('emailnonstudentcertificatelinktext', 'leeloolxpcert');
        }
