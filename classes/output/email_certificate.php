...
class email_certificate implements \renderable, \templatable {
    ...
    public function export_for_template(\renderer_base $renderer) {
        ...
        if ($this->isstudent) {
            $data->emailgreeting = get_string('emailstudentgreeting', 'leeloocert', $this->userfullname);
            $data->emailbody = get_string('emailstudentbody', 'leeloocert', $info);
            $data->emailbodyplaintext = get_string('emailstudentbodyplaintext', 'leeloocert', $info);
            $data->emailcertificatelink = new \moodle_url('/mod/leeloocert/view.php', array('id' => $this->cmid));
            $data->emailcertificatetext = get_string('emailstudentcertificatelinktext', 'leeloocert');
        } else {
            $data->emailgreeting = get_string('emailnonstudentgreeting', 'leeloocert');
            $data->emailbody = get_string('emailnonstudentbody', 'leeloocert', $info);
            $data->emailbodyplaintext = get_string('emailnonstudentbodyplaintext', 'leeloocert', $info);
            $data->emailcertificatelink = new \moodle_url('/mod/leeloocert/view.php', array('id' => $this->cmid));
            $data->emailcertificatetext = get_string('emailnonstudentcertificatelinktext', 'leeloocert');
        }
