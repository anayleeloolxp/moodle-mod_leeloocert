...
class report_table extends \table_sql {
    ...
    /**
     * @var int $leeloocertid The leeloo certificate id
     */
    protected $leeloocertid;
     ...
     */
    public function __construct($leeloocertid, $cm, $groupmode, $download = null) {
        parent::__construct('mod_leeloocert_report_table');
        ...
        $headers[] = get_string('receiveddate', 'leeloocert');
        $headers[] = get_string('code', 'leeloocert');

        // Check if we were passed a filename, which means we want to download it.
        if ($download) {
            $this->is_downloading($download, 'leeloocert-report');
        }
        ...
        if (!$this->is_downloading() && has_capability('mod/leeloocert:manage', $context)) {
            $columns[] = 'actions';
            $headers[] = '';
        }
        ...
        $this->leeloocertid = $leeloocertid;
        $this->cm = $cm;
    ...
    public function col_download($user) {
        ...
        $icon = new \pix_icon('download', get_string('download'), 'leeloocert');
        $link = new \moodle_url(
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        $total = \mod_leeloocert\certificate::get_number_of_issues($this->leeloocertid, $this->cm, $this->groupmode);

        $this->pagesize($pagesize, $total);

        $this->rawdata = \mod_leeloocert\certificate::get_issues(
            $this->leeloocertid,
            $this->groupmode,
            $this->cm,
            $this->get_page_start(),
