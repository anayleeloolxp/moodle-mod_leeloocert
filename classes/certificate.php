...
/**
 * Provides functionality needed by leeloocert activities.
 *
...
class certificate {
     ...
     *      If you want to display all leeloocerts on a page set this to 0.
     */
    const CUSTOMCERT_PER_PAGE = '50';

    /**
     * Handles setting the protection field for the leeloocert
     *
     * @param \stdClass $data
    ...
    /**
     * Handles uploading an image for the leeloocert module.
     *
    ...
    public static function upload_files($draftitemid, $contextid, $filearea = 'image') {
        ...
        file_save_draft_area_files($draftitemid, $contextid, 'mod_leeloocert', $filearea, 0);
    }
    ...
    /**
     * Returns a list of issued leeloocerts.
     *
     ...
     */
    public static function get_issues($leeloocertid, $groupmode, $cm, $limitfrom, $limitnum, $sort = '') {
        global $DB;
        ...
        // Add the conditional SQL and the leeloocertid to form all used parameters.
        $allparams = $conditionsparams + array('leeloocertid' => $leeloocertid);
        ...
        $sql = "SELECT $ufields, ci.id as issueid, ci.code, ci.timecreated
                  FROM {user} u
            INNER JOIN {leeloocert_issues} ci
                    ON u.id = ci.userid
                 WHERE u.deleted = 0
                   AND ci.leeloocertid = :leeloocertid
                       $conditionssql";
        if ($sort) {
            $sql .= "ORDER BY " . $sort;
        } else {
    ...
    /**
     * Returns the total number of issues for a given leeloocert.
     *
     * @param int $leeloocertid
     * @param \stdClass $cm the course module
     ...
     */
    public static function get_number_of_issues($leeloocertid, $cm, $groupmode) {
        global $DB;
    ...
    public static function get_conditional_issues_sql($cm, $groupmode) {
        ...
        // Get all users that can manage this leeloocert to exclude them from the report.
        $context = \context_module::instance($cm->id);
        $conditionssql = '';
        $conditionsparams = array();

        // Get all users that can manage this certificate to exclude them from the report.
        $certmanagers = array_keys(get_users_by_capability($context, 'mod/leeloocert:manage', 'u.id'));
    ...
    public static function issue_certificate($certificateid, $userid) {
        ...
        $issue = new \stdClass();
        $issue->userid = $userid;
        $issue->leeloocertid = $certificateid;
        $issue->code = self::generate_code();
        $issue->emailed = 0;
        $issue->timecreated = time();

        // Insert the record into the database.
        return $DB->insert_record('leeloocert_issues', $issue);
    }
    ...
    public static function generate_code() {
        ...
        while (!$uniquecodefound) {
            if (!$DB->record_exists('leeloocert_issues', array('code' => $code))) {
