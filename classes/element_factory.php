...
/**
 * Contains the factory class responsible for creating leeloo certificate instances.
 *
...
/**
 * The factory class responsible for creating leeloo certificate instances.
 *
 * @package    mod_leeloocert
 * @copyright  2017 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class element_factory {
     ...
     * @return \mod_leeloocert\element|bool returns the instance of the element class, or false if element
     *         class does not exists.
     */
    public static function get_element_instance($element) {
        // Get the class name.
        $classname = '\\leeloocertelement_' . $element->element . '\\element';

        $data = new \stdClass();
        $data->id = $element->id ?? null;
        $data->pageid = $element->pageid ?? null;
        $data->name = $element->name ?? get_string('pluginname', 'leeloocertelement_' . $element->element);
