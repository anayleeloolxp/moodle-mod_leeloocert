...
if ($tid) {
    ...
    // Set the page url.
    $pageurl = new moodle_url('/mod/leeloocert/edit.php', array('tid' => $tid));
} else { // Adding a new template.
    // Need to supply the contextid.
    $contextid = required_param('contextid', PARAM_INT);
    // Set the page url.
    $pageurl = new moodle_url('/mod/leeloocert/edit.php', array('contextid' => $contextid));
}
...
require_capability('mod/leeloocert:manage', $context);
...
if ($context->contextlevel == CONTEXT_SYSTEM) {
    ...
    if (!$tid) {
        $PAGE->navbar->add(get_string('editleeloocert', 'leeloocert'));
    } else {
        $PAGE->navbar->add(
            get_string('editleeloocert', 'leeloocert'),
            new moodle_url('/mod/leeloocert/edit.php', ['tid' => $tid])
        );
    }
}
        ...
        switch ($action) {
            ...
            case 'deleteelement':
                if (!empty($confirm)) { // Check they have confirmed the deletion.
                    ...
                    $deleting = true;
                    // Create the message.
                    $message = get_string('deleteelementconfirm', 'leeloocert');
                    // Create the link options.
                    $nourl = new moodle_url('/mod/leeloocert/edit.php', array('tid' => $tid));
                    $yesurl = new moodle_url(
                        '/mod/leeloocert/edit.php',
...
if ($tid) {
    $mform = new \mod_leeloocert\edit_form($pageurl, array('tid' => $tid));
    // Set the name for the form.
    $mform->set_data(array('name' => $template->get_name()));
} else {
    $mform = new \mod_leeloocert\edit_form($pageurl);
}

if ($data = $mform->get_data()) {
    // If there is no id, then we are creating a template.
    if (!$tid) {
        $template = \mod_leeloocert\template::create($data->name, $contextid);
    ...
    foreach ($data as $key => $value) {
        ...
        if (strpos($key, 'addelement_') !== false) {
            ...
            $url = new moodle_url('/mod/leeloocert/edit_element.php', $params);
            redirect($url);
        }
    }

    // Check if we want to preview this leeloo certificate.
    ...
    $url = new moodle_url('/mod/leeloocert/edit.php', array('tid' => $template->get_id()));
    redirect($url);
}
...
if ($tid && $context->contextlevel == CONTEXT_MODULE) {
    $loadtemplateurl = new moodle_url('/mod/leeloocert/load_template.php', array('tid' => $tid));
    $loadtemplateform = new \mod_leeloocert\load_template_form(
        $loadtemplateurl,
